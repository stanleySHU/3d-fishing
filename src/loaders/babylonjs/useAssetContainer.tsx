import { AbstractAssetTask, AssetsManager, IAssetsProgressEvent, TextureAssetTask, ContainerAssetTask } from '@babylonjs/core/Misc/assetsManager.js';
...
export type Task = BinaryTask | MeshTask | TextureTask | ContainerTask;
...
const getTaskKey = (task: Task): string | undefined => {
  switch (task.taskType) {
    ...
    case TaskType.Container:
      return `container${task.rootUrl}/${task.sceneFilename}`;
    default:
        ...
        if (key !== undefined && suspenseCache[key]) {
          ...
          switch (task.taskType) {
            ...
            case TaskType.Texture:
              ...
              break;
            case TaskType.Container:
              const containerTask = assetManager.addContainerTask(task.name, task.meshesNames, task.rootUrl, task.sceneFilename);
              newRequests.set(containerTask, task);
              break;
            default:
...
export const useOverrideAssetManager = useAssetManagerWithCache();
